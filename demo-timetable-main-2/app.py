    ...
    for dept_obj in depts:
        ...
        for sem in target_sems:
            try:
                ...
                tt_eng.generate(use_cpsat=False)
...
def hod_preference_window():
    ...
    pref_status = []
    for t in teachers:
        prefs = TeacherPreference.query.filter_by(teacher_id=t.id).all()
        odd_count  = len([p for p in prefs if _sem_is_odd(p.semester)])
        even_count = len([p for p in prefs if _sem_is_even(p.semester)])
        pref_status.append({
            'teacher': t,
            'odd_count': odd_count,
